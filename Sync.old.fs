...
module Sync =
    ...
    let (|WebEle|_|) ele = if (box ele) :? IWebElement then Some ele else None

    let (|SelectorString|_|) ele = 
        if box ele :? string then Some (element ele) else None
    ...
    type DriverState =
        | Searching of string
        | Acting of (IWebElement -> IWebDriver -> unit) 
